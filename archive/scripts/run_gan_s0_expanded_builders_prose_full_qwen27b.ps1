...
$experiment = "configs/experiments/gan_s0_expanded_builders_prose_full_validation_qwen27b_ollama.json"
$timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
$logDir = Join-Path $PWD "runs\overnight_logs"
New-Item -ItemType Directory -Force -Path $logDir | Out-Null
$logPath = Join-Path $logDir "gan_s0_expanded_builders_prose_full_qwen27b_$timestamp.log"
$pidPath = Join-Path $logDir "gan_s0_expanded_builders_prose_full_qwen27b_$timestamp.pid"
...
Write-Log "Gan S0 Qwen27b F0 expanded builders prose full validation run started"
