...
def gan_prompts_data(
    *,
    ...
    elif program_variant == GAN_FREQUENCY_S0_SUPPORT_AWARE_TARGET_SELECTOR_VARIANT:
        module_name = "GanFrequencyS0SupportAwareTargetSelectorModule"
        predictor_name = (
            "D1 deterministic date/event payload + deterministic temporal "
    ...
    elif program_variant == GAN_FREQUENCY_S0_EVIDENCE_FIRST_TARGET_SELECTOR_VARIANT:
        module_name = "GanFrequencyS0EvidenceFirstTargetSelectorModule"
        predictor_name = (
            "deterministic temporal candidates + G21 constructed answer options + "
            "dspy.Predict(evidence first target selection)"
        )
