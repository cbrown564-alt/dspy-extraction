"""Post-hoc G26 report for the Gan S0 Qwen selector replication."""
...
from clinical_extraction.evaluation.gan_g28_evidence_first_target_selector import (
    DEFAULT_G22_RUN,
    DEFAULT_G15_RUN,
)
...
G26_ARM_ID = "g26_qwen_selector_replication"
...
DEFAULT_G28_RUN = Path(
    "runs/gan_s0_g24_evidence_first_target_selector_gpt4_1_mini_standard50_20260529T131013Z"
)
...
DEFAULT_JSON_OUTPUT = Path(
    "docs/experiments/gan/gan_s0_g26_qwen_selector_replication_report_20260529.json"
)
DEFAULT_MARKDOWN_OUTPUT = Path(
    "docs/experiments/gan/gan_s0_g26_qwen_selector_replication_report_20260529.md"
)
MOTIVATING_OVERLAYS = (
    "target_selection",
...
def build_gan_g26_qwen_selector_replication_report(
    *,
    g26_run_dir: Path,
    ...
    """Build the G26 standard50 comparison and row-level before/after ledger."""

    arm_dirs = dict(arm_run_dirs or DEFAULT_BASELINE_RUNS)
    arm_dirs[G26_ARM_ID] = g26_run_dir
    ...
    report.update(
        {
            ...
            "decision_scope": "arm",
            "kanban_card": "G26 - Gan Qwen Selector Replication Batch",
            "comparison_group": (
                "gan_s0_g26_evidence_first_target_selector_qwen_standard50_v1"
            ),
            "g26_claim_scope": (
                "Matched Qwen3.6:35b replication for the evidence-first selector mechanism "
                "chosen by G24, with GPT-4.1-mini comparison held fixed."
            ),
            "fixed_controls": [
    ...
    predictions_by_id = _g26_predictions_by_id(g26_run_dir)
    report["challenge_overlays"] = _challenge_overlays(report)
    report["g26_diagnostics"] = _g26_diagnostics(
        report=report,
...
def write_report_artifacts(
    ...
    *,
    ...
    markdown_output.write_text(render_gan_g26_markdown(report), encoding="utf-8")


def render_gan_g26_markdown(report: dict[str, Any]) -> str:
    decision = report["decision"]
    diag = report["g26_diagnostics"]
    arm_ids = [arm["arm_id"] for arm in report["arms"]]
    lines = [
        "# Gan S0 G26 Qwen Selector Replication Report",
        "",
        ...
        "- Model/provider: Qwen3.6:35b / Ollama",
        ...
        "- Claim scope: matched Qwen replication of the evidence-first selector chosen by G24.",
        "",
        "## Decision",
        "",
    ...
    lines.extend(
        [
            "",
            "## G26 Trace Diagnostics",
            "",
            f"- Closed answer options present: {diag['closed_answer_options_present']['count']}/{diag['closed_answer_options_present']['denominator']}",
            f"- Selected closed option present: {diag['selected_closed_answer_option_present']['count']}/{diag['selected_closed_answer_option_present']['denominator']}",
            f"- Constructed-option rows: {diag['constructed_answer_options_present']['count']}/{diag['constructed_answer_options_present']['denominator']}",
            f"- Final label copied/matched trace: {diag['final_label_copied_from_selected_option']['count']}/{diag['final_label_copied_from_selected_option']['denominator']}",
    ...
    lines.extend(
        [
            ...
            "## Before/After Ledger",
            "",
            "| Record | Gold | G19 classes | G17 bucket | G21 constructed exact | Selected option | Before paper monthly | G26 paper monthly | Tags |",
            "| --- | --- | --- | --- | ---: | --- | --- | ---: | --- |",
        ]
    )
    for row in report["before_after_ledger"]:
        selected = row["g26_selection"]
        ...
        lines.append(
            ...
            f"{before} | "
            f"{_yn(row['after']['g26']['paper_monthly_match'])} | "
    ...
    lines.extend(
        [
            ...
            "## Interpretation",
            "",
            "- G26 is evaluated as a matched Qwen3.6:35b replication of the evidence-first selector (G28).",
            "- Compare the Qwen row-level successes and errors to the GPT-4.1-mini comparison group (G28).",
            "",
        ]
    )
    return "\n".join(lines)
...
def _g26_predictions_by_id(run_dir: Path) -> dict[str, DocumentPrediction]:
...
def _g26_diagnostics(
    *,
    ...
    for row in report["rows"]:
        ...
        g26_row = row["arms"][G26_ARM_ID]
        predicted_label = g26_row.get("predicted_label")
...
def _before_after_ledger(
    *,
    ...
    baseline_arm_ids = [
        arm["arm_id"] for arm in report["arms"] if arm["arm_id"] != G26_ARM_ID
    ]
    ...
    for row in report["rows"]:
        ...
        after = {"g26": _compact_arm_result(g26_row)}
        g26_paper = after["g26"]["paper_monthly_match"]
        tags = _ledger_tags(before, g26_paper)
...
def _ledger_tags(before: dict[str, dict[str, Any]], g26_paper: bool) -> list[str]:
    tags: list[str] = []
    improved = [
        arm_id
        for arm_id, row in before.items()
        if g26_paper and not row["paper_monthly_match"]
    ]
    regressed = [
        arm_id
        for arm_id, row in before.items()
        if not g26_paper and row["paper_monthly_match"]
    ]
    if improved:
        tags.append("g26_corrects_baseline_miss")
    if regressed:
        tags.append("g26_regresses_from_baseline_correct")
...
def _pairwise_deltas(report: dict[str, Any]) -> dict[str, Any]:
    ...
    for arm_id in baseline_arm_ids:
        ...
        for row in report["rows"]:
            g26_match = bool(
                row["arms"][G26_ARM_ID]["scores"]["paper_reproduction"][
                    "monthly_frequency_match"
                ]
            )
            baseline_match = bool(
            ...
            if g26_match and not baseline_match:
                improved.append(row["record_id"])
            elif not g26_match and baseline_match:
                regressed.append(row["record_id"])
        deltas[arm_id] = {
            "g26_correct_baseline_wrong": improved,
            "g26_wrong_baseline_correct": regressed,
        }
    return deltas
...
def _decision(report: dict[str, Any]) -> dict[str, Any]:
    ...
    g26_summary = arm_summaries[G26_ARM_ID]["paper_reproduction"]
    g26_correct = round(
        g26_summary["monthly_frequency_accuracy"] * g26_summary["denominator"]
    )
    baseline_ids = [arm_id for arm_id in arm_summaries if arm_id != G26_ARM_ID]
    baseline_correct = {
        arm_id: round(
    ...
    lift_passed = g26_correct >= 43
    gate_passed = (
        (g26_correct >= 43 or (g26_correct >= 39 and not overlay_regressions and len(g17_regressions) <= 1))
        and trace_complete
    )
    return {
        "g26_paper_monthly_correct": g26_correct,
        ...
        "recommendation": (
            ...
            if gate_passed
            else "do_not_full_validate_g26"
        ),
        "rationale": _decision_rationale(
            g26_correct=g26_correct,
...
def _motivating_overlay_regressions(
    ...
    for name in MOTIVATING_OVERLAYS:
        ...
        g26_count = overlay.get(G26_ARM_ID, {}).get("paper_monthly", {}).get("correct", 0)
        baseline_counts = {
...
def _g17_regressions_against_builder(report: dict[str, Any]) -> list[str]:
    ...
    for row in report["before_after_ledger"]:
        ...
        if not builder:
            continue
        if builder["paper_monthly_match"] and not row["after"]["g26"]["paper_monthly_match"]:
...
def _decision_rationale(
    *,
    ...
    parts = [
        (
            f"G26 paper-monthly count is {g26_correct} versus best baseline "
