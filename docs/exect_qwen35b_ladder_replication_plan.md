...
**Do not compare Qwen ladder metrics to GPT ladder metrics as a single experiment.** Same deterministic scorers and splits; different model track. See `docs/kanban_plan.md` (active board) and `docs/kanban_frozen_threads_history.md` (hosted GPT anchors).
...
- Optimizers on Qwen (`BootstrapFewShot`, GEPA) per `docs/qwen_dspy_latency_policy.md`.
...
5. **Prior Qwen themes** — `docs/previous_exect_error_analysis.md`, `docs/prior_prompt_error_analysis_synthesis.md` (seizure granularity, ILAE specificity).
...
Update `docs/kanban_plan.md` (active board) when Qwen phases complete; add Qwen anchor rows to `docs/kanban_frozen_threads_history.md` when replication stabilizes.
