...
| Medication current-Rx | coverage gate passed / isolated ceiling open | E3 annotation-derived current-Rx payload reproduces 47/47 validation medication gold labels across 29/29 gold-bearing documents. | Use the payload for an isolated medication ceiling or stack-interference probe; keep note-surface lifecycle candidates diagnostic. |
| Medication lifecycle / temporality | diagnostic / deferred | E3 lifecycle inventory exposes planned (11), previous (9), taper/stop (8), dose-line (131), dose-only/unknown (60), non-ASM (4), and unknown-temporality (116) rows; note-surface current candidates cover only 22/47 medication gold labels. Prescription JSON still lacks native temporality. | Define a benchmark-facing versus clinical-diagnostic target before any model-backed temporality run. |
...
| Medication current-Rx payload | diagnostic coverage gate / isolated ceiling open | ExECTv2; `exectv2_fixed_v1:validation`; 40 documents | none | Artifact `experiments/exect/exect_medication_current_rx_lifecycle_payload_audit_20260528.json` | No-model payload-vs-gold audit over current prescription policy; annotated current-Rx uses benchmark prescription annotations | Annotated current-Rx payload reproduces 47/47 validation medication gold labels, but an isolated model ceiling still has not been run. |
| Medication lifecycle / temporality | diagnostic / deferred | ExECTv2; `exectv2_fixed_v1:validation`; 40 documents | none | Same E3 medication payload artifact | Lifecycle rows are note-surface diagnostics; prescription JSON has no native temporality column | Do not make lifecycle a benchmark-facing metric until E5 decides target policy and scorer caveats. |
