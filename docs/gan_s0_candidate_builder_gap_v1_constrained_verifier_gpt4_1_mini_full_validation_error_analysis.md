# Gan S0 Error Analysis: gan_s0_candidate_builder_gap_v1_constrained_verifier_gpt4_1_mini_full_validation

## Run

- Artifact directory: `runs\gan_s0_candidate_builder_gap_v1_constrained_verifier_gpt4_1_mini_full_validation_20260523T161534Z`
...
| --- | ---: | ---: | ---: |
| normalized_label | 65.9% | 197 | 299 |
| monthly_frequency | 74.6% | 223 | 299 |
| purist_category | 80.6% | 241 | 299 |
| pragmatic_category | 84.3% | 252 | 299 |
...
- Overall operational failure rate: 25.4% (76 failures)

| Stratum | All records | Valid scored | Operational failure rate | Monthly (valid only) |
| --- | ---: | ---: | ---: | ---: |
| hard_case=true | 39 | 39 | 20.5% | 79.5% |
| hard_case=false | 260 | 260 | 26.2% | 73.8% |
| row_ok=true | 287 | 287 | 26.5% | 73.5% |
| row_ok=false | 12 | 12 | 0.0% | 100.0% |
| gold_pragmatic=frequent | 152 | 152 | 32.9% | 67.1% |
| gold_pragmatic=infrequent | 51 | 51 | 37.3% | 62.7% |
| gold_pragmatic=no_seizure_information | 56 | 56 | 1.8% | 98.2% |
| gold_pragmatic=unknown | 40 | 40 | 15.0% | 85.0% |
...
- Gold label present in candidate set: 39/299 (13.0%)
...
| --- | --- | ---: | ---: |
| `1111` | all_four_match | 197 | 65.9% |
| `0111` | monthly_purist_pragmatic_not_normalized | 26 | 8.7% |
| `0001` | pragmatic_only | 11 | 3.7% |
| `0000` | all_four_wrong | 47 | 15.7% |
| `0011` | purist_pragmatic_not_monthly | 18 | 6.0% |

### Logical containment on this run

- Normalized exact (197 records): always co-occurs with monthly, Purist, and Pragmatic match.
- Monthly match (223 records): always implies Purist and Pragmatic; 197 also have normalized exact.
- Purist match (241 records): always implies Pragmatic; 223 also match monthly.
- Pragmatic-only wins (pattern `0001`): 11 records.
- Purist-without-monthly (pattern `0010`/`0011`): 0 + 18 records.

### Boundary cases

- **purist_bin_boundary_within_pragmatic**: 11
- **pragmatic_match_monthly_divergence**: 29
- **purist_match_monthly_divergence**: 18
- **monthly_match_label_surface_mismatch**: 26
...
| Failure class | Count |
| --- | ---: |
| other_semantic_mismatch | 28 |
| pragmatic_match_monthly_divergence | 18 |
| purist_bin_boundary_within_pragmatic | 9 |
...
| seizure_free_to_no_reference_monthly_match | 2 |
...
The leading benchmark-severe failure class on this run is `other_semantic_mismatch` (28 scored misses). These are the first prompt or verifier targets; lower-tier metric wins should not hide them.

Cluster-format errors account for 10 scored misses, split between incomplete cluster labels (invalid), cluster structure swaps, and cluster collapsed to simple rates.

There are 11 **pragmatic-only** successes: same coarse bucket (infrequent vs frequent vs unknown vs no information) but wrong monthly value and Purist bin. These are clinically misleading if only Pragmatic accuracy is reported.

Purist-without-monthly cases: 18; pragmatic-without-monthly: 29. These arise when different labels land in the same bin but convert to different seizures/month.
...
| gan_4709 | scored | Y | Y | Y | Y | all_metrics_match | multiple per day | multiple per day |
| gan_9566 | scored | Y | Y | Y | Y | all_metrics_match | unknown | unknown |
| gan_12679 | scored | Y | Y | Y | Y | all_metrics_match | 1 per day | 1 per day |
| gan_1584 | scored | N | N | N | N | other_semantic_mismatch | 11 per month | unknown |
...
| gan_14689 | scored | N | N | N | N | frequent_undercalled | 3 per 2 month | 2 per 2 month |
| gan_4100 | scored | N | N | Y | Y | pragmatic_match_monthly_divergence | 1 per 2 to 3 week | 1 per 3 week |
...
| gan_14628 | scored | N | N | N | N | other_semantic_mismatch | 2 per 2 month | unknown |
...
| gan_6131 | scored | N | N | N | N | unknown_as_quantified_rate | unknown | 1 per 12 month |
...
| gan_9424 | scored | N | N | Y | Y | pragmatic_match_monthly_divergence | 10 per 9 month | 11 per 9 month |
...
| gan_7818 | scored | N | Y | Y | Y | seizure_free_label_shape_mismatch | seizure free for 2 year | seizure free for multiple month |
...
| gan_1640 | scored | N | N | Y | Y | pragmatic_match_monthly_divergence | 5 per week | 2 to 3 per week |
...
| gan_3864 | scored | N | N | N | N | other_semantic_mismatch | 3 per day | unknown |
...
| gan_4996 | scored | Y | Y | Y | Y | all_metrics_match | seizure free for 16 month | seizure free for 16 month |
...
| gan_13450 | scored | N | Y | Y | Y | seizure_free_label_shape_mismatch | seizure free for 1 year | seizure free for multiple year |
...
| gan_8858 | scored | Y | Y | Y | Y | all_metrics_match | seizure free for multiple month | seizure free for multiple month |
...
| gan_3329 | scored | N | N | N | N | other_semantic_mismatch | 2 to 3 per day | unknown |
...
| gan_16750 | scored | N | N | Y | Y | pragmatic_match_monthly_divergence | 6 per 7 month | 2 per 8 month |
...
| gan_16523 | scored | N | N | N | N | other_semantic_mismatch | 1 per 5 day | unknown |
...
| gan_16645 | scored | N | N | N | N | frequent_overcalled | 5 per 7 month | 3 cluster per month, multiple per cluster |
...
| gan_1497 | scored | N | N | Y | Y | pragmatic_match_monthly_divergence | 3 per month | 2 per month |
...
| gan_5866 | scored | N | N | N | N | other_semantic_mismatch | 4 per 6 week | unknown |
| gan_10074 | scored | N | N | N | Y | cluster_semantic_mismatch | 5 cluster per month, multiple per cluster | 1 cluster per month, multiple per cluster |
...
| gan_16529 | scored | N | N | N | N | other_semantic_mismatch | 1 per 5 day | unknown |
| gan_6763 | scored | N | N | N | N | frequent_undercalled | 1 per week | 2 to 3 per 3 month |
...
| gan_16422 | scored | N | N | Y | Y | pragmatic_match_monthly_divergence | 1 per 2 to 3 day | 1 to 3 per 3 day |
...
| gan_13149 | scored | N | N | N | N | frequent_overcalled | 3 per year | 3 per week |
...
| gan_14562 | scored | N | N | Y | Y | pragmatic_match_monthly_divergence | 3 per 6 month | 2 per 7 month |
