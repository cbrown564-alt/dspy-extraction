...
- `docs/exect_gold_label_audit.md`: JSON primary source, certainty threshold, seizure types from `Diagnosis` rows, diagnosis specificity collapse, and warning flags.
- `docs/gan_2026_label_audit.md`: Gan gold source, reference as cross-check, plural unit normalization, year/month equivalence, cluster conversion, hard-case flags, and special labels.
- `docs/agents.md`: deterministic loaders, validators, normalizers, and scorers around LLM components; no silent scorer semantic changes.
