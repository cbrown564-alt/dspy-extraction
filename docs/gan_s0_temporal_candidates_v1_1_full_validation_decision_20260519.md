...
Date: 2026-05-19  
Status: **Pending** — awaiting `runs/gan_s0_qwen35b_temporal_candidates_verify_repair_full_validation_guardrails_20260519T183614Z`
...
| Role | Path |
| --- | --- |
| Candidate run | `runs/gan_s0_qwen35b_temporal_candidates_verify_repair_full_validation_guardrails_20260519T183614Z` |
...
**E regression guard (veto / tier 3):** wrong YTD, stripped cluster, `unknown`→`no reference`, short seizure-free — shapes from `docs/gan_s0_temporal_candidate_pivot_20260519.md`.
...
**Allowed:** ExECT S0/S1 monolithic **cap-25** only (`configs/experiments/exect_s0_s1_validation_cap25_gpt4_1_mini.json`; anchor inspection `docs/exect_s0_s1_validation_cap25_inspection.md`). Comparison anchor only — no section-aware promotion, no scorer changes.

**Blocked until tier signed:** B1/B2/ReAct/hosted Gan port; Gan prompt edits; GEPA; Gemini verify-repair scale-up; gold-ambiguity implementation.

## Promotion packet checklist

- [ ] **A** — Full-split analyzer markdown (299 records)
- [ ] **B** — Same run, 14-record slice via `--record-ids-file` (failure_mode breakdown)
- [ ] **C** — Stratified table: `gold_pragmatic=infrequent`, `hard_case`, invalid/abstain
- [ ] **D** — Anchor comparison table (this doc § Headline metrics)
- [ ] **E** — Full-split regression guard checklist (verifier repair failure shapes)
- [ ] **F** — Defer hosted port / ReAct / GEPA until tier ≥ 2 on slice B
...
uv run python scripts/analyze_gan_frequency_run.py $run `
  ...
  --markdown docs/gan_s0_qwen35b_temporal_candidates_verify_repair_full_validation_guardrails_error_analysis.md

# B — 14-record regression slice from same artifacts
uv run python scripts/analyze_gan_frequency_run.py $run `
  --split-name gan_2026_fixed_v1:validation `
  --record-ids-file data/fixtures/gan_s0_qwen_error_regression_slice.json `
  --markdown docs/gan_s0_qwen35b_temporal_candidates_verify_repair_regression_slice_from_full_validation_error_analysis.md
```
...
| --- | ---: | ---: | ---: |
| Records analyzed | TBD | 299 | 299 |
| Schema-valid rate | TBD | — | 96.7% |
| Monthly-frequency accuracy | TBD | 55.9% | 65.4% |
| Purist category accuracy | TBD | 61.9% | 72.7% |
| Pragmatic category accuracy | TBD | 70.5% | 79.2% |
| Evidence quote support | TBD | 99.6% | 92.7% |

## Slice replay (fill after B)

| Stratum | Target | Result |
| --- | --- | --- |
| Original policy/cluster rows (10) | 10/10 monthly | TBD |
| Infrequent quantified (4) | ≥3/4 monthly (gate history: 4/4 on dedicated slice run) | TBD |
| Per `failure_mode` | No cluster/YTD/unknown regressions vs pivot doc | TBD |

## Decision

- **Assigned tier:** TBD  
- **Signed:** TBD  
- **Next pull:** TBD (tier 1 doc only / B1 / B2 / guard patch)
