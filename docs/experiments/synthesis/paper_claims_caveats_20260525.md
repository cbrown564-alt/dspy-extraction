...
7. Several tempting mechanism arms are rejected as tested, including ExECT S5 per-family parallel decomposition, stacked strict frequency verification, high-precision frequency-candidate pruning, medication temporal guarding inside the S5 core surface, Gan unknown-overuse prompting, and Gan GEPA G1/G2.
...
2. Decide whether the paper needs a best-closed anchor on S4 or S5; only run one if it would change a claim.
