...
This note reviews the synthesis-backed Gan S0 full-validation run before changing the next experiment target. It is grounded in `docs/datasets/gan/gan_2026_label_audit.md`: `seizure_frequency_number[0]` remains the gold label, `reference` remains a secondary cross-check, and evidence quote support remains diagnostic rather than benchmark-facing label scoring.
