...
Last updated: 2026-05-29 after G14 temporal anchoring result
...
3. Treat G13 as the source-level gate baseline before G15: do not attribute
   unclear-frequency or seizure-free gate misses to target selection until the
   G13 false-positive/false-negative rows are accounted for.
...
4. Treat G14 as the temporal anchoring diagnostic baseline: the current
   ...
   aggregation-aware answer construction plus LLM target selection.
5. Use G6 surfaces for any new selector/adjudicator execution: the old enriched
   ...
   overlays.
6. Keep the G4 negative result, G5 scorer-mode forensics, and G8/G9 reports as
   required context for any seizure-free, quantified-rate, unknown,
   no-reference, or candidate-constrained selector.
7. Keep arithmetic and broad relative-anchor guardrails diagnostic-only until a
   seizure-specific parser exists.
8. Use the G5 rescore pack for synthetic-only paper-facing tables; Real(300)
