...
Date: 2026-05-21  
Status: **Historical snapshot** — superseded for research doctrine by `docs/hybrid_pipeline_research_pivot_20260521.md`  
...
| **Compile-time search** | Optimizer role vs manual engineering | `docs/dspy_optimizer_investigation_20260521.md` |
...
**Artifact:** `exect_s1_full_ladder_d1_cap25_gpt4_1_mini_20260521T003704Z` — `docs/exect_s1_full_ladder_gpt_validation_v1_inspection_20260521.md`
...
- **Inline benchmark bridges** are load-bearing: bridge-free L1 raw **68.6%** full micro vs **92.3%** with bridges (~24pp), measured in interleaving v2 (`docs/exect_s1_interleaving_gpt_validation_v2_inspection_20260520.md`).
- **S1 post-bridge H1 as a new intervention:** **Reject** — reproduces production path; diagnostic only (`docs/exect_negative_probe_synthesis_20260520.md`).
- **Second-pass verify-repair on S1 (cap-25):** **Reject** — 86.4% vs 95.8% single-pass micro (−9.4pp) (`docs/exect_s1_gpt_factor_isolation_cap25_inspection_20260521.md`).

**What we know — ExECT S4:**

- Medication-temporality **post** classifier: precision +10.1pp, F1 −6.6pp (recall collapse) — **reject** broad abstention (`docs/exect_s4_temporality_deterministic_gpt_inspection_20260520.md`).
...
**Rationale:** Optimizers are wired correctly; mismatch is **failure mode** (benchmark alignment, multi-step programs) and **missing ladder rungs**, not broken DSPy integration (`docs/dspy_optimizer_investigation_20260521.md`).
...
The intervention matrix in `docs/hybrid_component_taxonomy_decision_20260520.md` is the right mental model: the same knowledge (temporal rules, vocab, evidence) must be compared at **pre / during / tool_during / post**, not as “we added normalization.”
...
**Not in flight:** No model-backed runs active per `docs/kanban_plan.md`.
...
5. **Published benchmarks:** No paper-safe ExECT/Gan external claims until CUI scoring and real-set access unblock Phase 3 in `docs/kanban_plan.md`.
...
What felt like drift is **fragmentation of narrative**, not abandonment of the thesis. `docs/kanban_plan.md` optimizes **execution**; `docs/hybrid_component_taxonomy_decision_20260520.md` optimizes **ontology**; this document optimizes **synthesis**.
...
- Bridge gains confound **clinical validity** vs **benchmark alignment** — scorer and audit caveats in `docs/exect_gold_label_audit.md`.
...
3. **Do not run** closed shapes in `docs/exect_negative_probe_synthesis_20260520.md` without a new `varied_factor` and preregistration.
4. **Consolidation:** Add taxonomy blocks to any new inspection doc; update `docs/experiment_taxonomy_research_synthesis_20260520.md` § “Direct Answers” with ladder + Lane A citations (or supersede with this doc as the living synthesis).
...
| --- | --- |
| `docs/hybrid_component_taxonomy_decision_20260520.md` | Ontology and intervention matrix |
| `docs/experiment_taxonomy_research_synthesis_20260520.md` | Registry-level answers (2026-05-20) |
| `docs/exect_negative_probe_synthesis_20260520.md` | Closed ExECT mechanisms |
| `docs/exect_s1_full_ladder_gpt_validation_v1_inspection_20260521.md` | D1/L0/L1/L1+policy decomposition |
| `docs/exect_s1_gpt_factor_isolation_cap25_inspection_20260521.md` | S1 prompt/verification/evidence |
| `docs/gan_s0_lane_a_gpt_cap25_inspection_20260521.md` | Gan verification/evidence/prompt nulls |
| `docs/dspy_optimizer_investigation_20260521.md` | Optimizer ladder and rejects |
| `docs/exect_s1_seizure_prompt_policy_qwen_v1_inspection_20260520.md` | Qwen v4_11 hold / promote blocked |
| `docs/kanban_plan.md` | Active/complete status as of 2026-05-21 |
