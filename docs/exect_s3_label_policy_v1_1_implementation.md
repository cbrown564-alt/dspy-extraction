...
Full validation error read (`docs/exect_s3_validation_full_gpt4_1_mini_inspection_20260520.md`) showed **investigation label-format collapse** (10.3% F1 vs 90.0% S2 full on the same 40 records): the nine-family pass caused the model to emit imaging prose instead of benchmark `modality+result` strings. Frozen S2 bridges in `exect_s2.py` were unchanged.
