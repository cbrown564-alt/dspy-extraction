...
| --- | --- | --- | --- | --- |
| **E5 - ExECT S5 Axis 1 / Axis 2 grid** | Run the ExECT S5 Axis 1 / Axis 2 pipeline decomposition grid (stage counts and deterministic-vs-LLM placement) on the validation split now that candidate recall is 100% stable. | S4 frequency recall iteration (E6) complete | yes | Preregistered S5 comparison controls, stage-graph configs, and scorer output |
...
| **Gan S0 multi-year seizure-free candidate helper** | Implement narrow candidate helper for `seizure free for multiple year` wording to resolve gaps for `gan_13574` and `gan_13598` identified in pilot. | [gan_s0_cursor_sdk_mutation_pilot_review_20260524.md](file:///c:/Users/cbrow/Code/dspy-extraction/docs/experiments/gan/gan_s0_cursor_sdk_mutation_pilot_review_20260524.md) | yes | Focused validation checks passing in [test_gan_temporal_candidates.py](file:///c:/Users/cbrow/Code/dspy-extraction/tests/test_gan_temporal_candidates.py) and new regression cases |
