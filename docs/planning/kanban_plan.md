...
| --- | --- | --- | --- | --- |
| **Triage paper & hygiene drafts** | Parallel triage of newly generated paper narrative and documentation hygiene reports using the Cursor SDK. Older paper/hygiene outputs are now `keep_as_lead`; fresh table source-map and model-compatibility outputs are in active review. | SDK runs `20260524T093308Z`, `20260524T093256Z`, `20260524T131249Z`, and `20260524T131358Z` | yes | Review queue updated; source files indexed |
| **Cursor SDK GEPA workstream stub triage** | Cursor SDK `inspection-draft` run `20260524T125735Z` passed runner checks but produced a zero-byte artifact; treat as `stub/reject` and do not use for source claims. | None | yes | Review queue records the stub; workstream cites primary docs instead |
...
| **Paper result table freeze - source-map draft** | Produce a review-only source map for table rows before manual table freeze. | Completed SDK run `20260524T131249Z`: `uv run python scripts/cursor_sdk_workflows.py paper-synthesis --topic "paper result table source map for current operational defaults and caveats"`. Require review against `docs/experiments/synthesis/experiment_registry.json`, promoted inspection docs, and `runs/*/metrics.json`. | Draft `docs/experiments/cursor_sdk_drafts/20260524T131249Z_paper_synthesis_draft.md` exists, the JSONL ledger records `human_review_status: needs_review`, and the review queue has an `active_review` row for the draft. | Cursor can prepare the source map only. The frozen manuscript table must be manually assembled and checked against primary artifacts. |
...
| **Model compatibility stale-check** | Generate a non-mutating compatibility report for any newly added provider/model config before implementation work. | Completed SDK run `20260524T131358Z`: `uv run python scripts/cursor_sdk_workflows.py model-compatibility --topic "stale-check newly added model configs against hosted/local adapter rules"`. Review `configs/models/`, `src/clinical_extraction/llms.py`, and `docs/policies/model_config_compatibility_backlog_20260524.md`. | Draft `docs/workstreams/cursor_sdk/compatibility/20260524T131358Z_model_compatibility_report.md` exists, the JSONL ledger records `human_review_status: needs_review`, and the review queue has an `active_review` row for the draft. | Cursor may propose validations/wrapper changes only. Runtime adapter edits and tests require a normal implementation task. |
...
2. **Cursor SDK end-to-end triage pull.** Close the paper/hygiene active-review items and the GEPA zero-byte stub using the instructions above; no source promotion during the SDK pass.
3. **Provider adapter validation (B2/B3).** Address compatibility and environment backlog items from [model_config_compatibility_backlog_20260524.md](file:///c:/Users/cbrow/Code/dspy-extraction/docs/policies/model_config_compatibility_backlog_20260524.md).
