...
| `docs/policies/model_config_smoke_tests.md` | Smoke outcomes, known provider failures |
| `.agents/skills/model-config-compatibility/SKILL.md` | Workflow, known failure modes |
| `docs/policies/constrained_json_decoding_strategy.md` | DSPy JSONAdapter vs ChatAdapter fallback policy |
| `docs/policies/qwen_dspy_latency_policy.md` | Ollama `think=false`, `num_ctx` + `max_tokens` coupling |
...
| `src/clinical_extraction/experiments/runner.py` | Production path: `build_dspy_lm`; GEPA reflection LM override |
...
**Missing context (not read):** LiteLLM version-specific parameter names for GPT-5.5 reasoning controls, Anthropic extended-thinking defaults for `claude-sonnet-4-6`, and live Ollama OpenAI-compat JSON-schema support per tag. Claims about those behaviors are flagged as uncertain.
...
**Fact:** Experiment runs call `build_dspy_lm` only (`runner.py` L489–497; `scripts/smoke_gan_s0_adapter.py` L48). `build_chat_adapter` is tested but not used on the experiment hot path.

**Fact:** Provider routing differs between paths (`llms.py`):

| Provider | `build_dspy_lm` | `build_chat_adapter` |
|----------|-----------------|----------------------|
| `ollama` | LiteLLM `ollama_chat/{model}`; forces `extra_body={"think": False, …}` (L272–285) | OpenAI-compat `POST …/chat/completions` at `localhost:11434/v1`; no `think:false` |
| `gemini` | LiteLLM `gemini/{model}`; passes `reasoning_effort` (L236–254) | OpenAI-compat Gemini proxy URL; **no** `reasoning_effort` |
| `anthropic` | LiteLLM `anthropic/{model}`; `extra_body` nested in kwargs (L256–268) | OpenAI-compat Anthropic URL; `extra_body` merged flat into payload (L155) |
| `openai` | LiteLLM `openai/{model}` (L287–299) | Same OpenAI-compat adapter |

**Interpretation (decision scope: `mechanism`):** Config fields like `reasoning_effort` and Ollama `options.num_ctx` behave differently depending on which adapter is invoked. Production runs are mostly safe because experiments use `build_dspy_lm`, but config validation that only exercises `build_chat_adapter` (`test_model_comparison_configs.py`) can give a false “compatible” signal for Ollama/Gemini.

---

### Local Qwen (Ollama) configs

| Config file | Model tag | `max_tokens` | `num_ctx` (`extra_body.options`) | `timeout_seconds` | `num_retries` |
|-------------|-----------|--------------|-------------------------------------|-------------------|---------------|
| `gan_s0_qwen9b_ollama.json` | `qwen3.5:9b` | 1536 | *(none — Ollama default 4096)* | 600 | 0 |
...
- `build_dspy_lm` always injects `think: False` for Ollama (`llms.py` L272–279; `test_llm_adapters.py` L67–81). Documented rationale: OpenAI-compat `/v1` route returned hidden reasoning with empty final content (`qwen_local_latency_experiment_20260518.md` L20; `qwen_dspy_latency_policy.md` L13–16).
- `num_retries=0` on all Qwen configs; hosted configs omit the field and inherit default `3` (`llms.py` L41).
- Startup validation rejects `max_tokens > 4096` without `extra_body.options.num_ctx` (`llms.py` L56–62; `test_llm_adapters.py` L89–96).
...
|-----|-------|----------|----------|
| Gan smoke configs (`gan_s0_qwen27b_ollama.json`, `gan_s0_qwen35b_ollama.json`) use `max_tokens=256` with no `num_ctx` | `operational` | Truncation / schema-invalid outputs if program variant adds reasoning or long outputs | `qwen_local_latency_experiment_20260518.md` L30–32; configs |
...
| Max-budget configs (`*_max81920.json`) not for ordinary smokes | `mechanism` | Mis-applying them changes latency comparability | `qwen_dspy_latency_policy.md` L25–26 |
| `gan_s0_qwen9b_ollama_max81920.json` uses `timeout_seconds=600` with `num_ctx=262144` | `operational` | May timeout on long ExECT notes; 9b ExECT config uses 1800 s | Config comparison |
...
- Code auto-defaults `reasoning_effort="minimal"` when model string contains `gemini-3` and field is null (`llms.py` L247–251; `test_llm_adapters.py` L54–64).
...
- GPT-5.5 rejected `temperature=0`; config sets `"temperature": null`; adapter omits key when null (`model_config_smoke_tests.md` L67–69; `llms.py` L50–55, L151–152, L293–294; `test_llm_adapters.py` L84–86, L212–230).
...
| Gap | Scope | Notes |
|-----|-------|-------|
| `build_dspy_lm` OpenAI path does not map `reasoning_effort` or `extra_body` | `open` | If GPT-5.5 gains reasoning-only parameters, configs cannot express them today (`llms.py` L287–299) |
| GEPA runs set `reflection_config.temperature = 1.0` when base config has non-null temperature (`runner.py` L305–309) | `blocked` / `arm` | **Uncertain** whether GPT-5.5 rejects `temperature=1.0`; would surface only on GEPA experiments |
...
- `build_dspy_lm` passes `extra_body` as nested `kwargs["extra_body"]` (`llms.py` L267–268). Unit test shows chat adapter can send `thinking: {"type": "disabled"}` (`test_llm_adapters.py` L233–251), but **config does not set this**.
...
| Chat vs DSPy path both differ from native Messages API | `mechanism` | DSPy uses LiteLLM native prefix; chat adapter uses OpenAI-compat URL (`llms.py` L211–212) |
...
*Draft only — not implemented. Does not change scorer semantics, dataset policy, registry rows, or source-of-truth docs.*
...
**Problem:** `max_tokens=81920` without `num_ctx` still ran at Ollama default 4096 in at least one 9B run (`qwen_local_latency_experiment_20260518.md` L93–95). Partial enforcement exists today (`llms.py` L56–62) but does not cover timeout pairing.
...
def validate_ollama_context_budget(self) -> LLMProviderConfig:
    ...
    if num_ctx and num_ctx >= 131072 and self.timeout_seconds < 1800:
        raise ValueError(
...
**Problem:** `runner.py` L305–309 forces `temperature=1.0` for GEPA reflection when base config has non-null temperature.

```python
# Draft: runner.py (conceptual) — matches existing guard intent
...
**Extension (draft):** For GPT-5-family models, consider returning `base` unchanged or using a separate reflection model config path rather than forcing `temperature=1.0`.
...
### 5. OpenAICompatibleChatAdapter provider guard (chat path only)

**Problem:** `OpenAICompatibleChatAdapter` always sends `response_format.type=json_schema` with `strict=True` when a schema is provided (`llms.py` L156–164). Ollama and some proxies may reject this.

```python
# Draft: provider-aware structured output in OpenAICompatibleChatAdapter
def _structured_output_payload(
    provider: str, response_schema: dict[str, Any] | None
) -> dict[str, Any]:
    if response_schema is None:
        return {}
    if provider == "ollama":
        # Prefer ChatAdapter-style prompt fallback; do not send strict json_schema
        return {}
    return {
        "response_format": {
            "type": "json_schema",
...
**Decision scope:** `mechanism` — only relevant if chat adapter becomes production path.

---

### 6. Configuration recommendations (no code)
...
| GPT-5.5 | `"temperature": null`; avoid GEPA until reflection override is verified safe | `gan_s0_gpt5_5_openai.json`, `runner.py` L305–309 |
| Gemini 3 Flash | Keep `reasoning_effort: "minimal"`; do not copy OpenAI reasoning params | `gan_s0_gemini3_flash.json`, skill L28 |
| Anthropic Sonnet 4.6 | Consider `"extra_body": {"thinking": {"type": "disabled"}}` if extended thinking is default | `test_llm_adapters.py` L233–251 — **behavior uncertain on live API** |
...
| Rule | Scope | Action |
|------|-------|--------|
| If `model` matches `gpt-5*` and `temperature == 0.0`, reject with message pointing to `gan_s0_gpt5_5_openai.json` | `operational` | Already enforced (`llms.py` L50–55) |
| If `model` matches `gpt-5*` and experiment uses GEPA optimizer, warn or block until reflection temperature override is safe | `blocked` | `runner.py` L305–309 |

### Gemini

| Rule | Scope | Action |
|------|-------|--------|
| If `provider == "gemini"` and `"gemini-3" in model` and `reasoning_effort is None`, info-log auto-default `"minimal"` | `operational` | Matches `llms.py` L247–251 |
...
| If `max_tokens > 4096`, require `extra_body.options.num_ctx` | `operational` | Already enforced (`llms.py` L56–62) |
...
| Qwen 35b smoke not re-verified on all target hosts | `blocked` | Mac blocked; Windows status in smoke doc but not re-run in this scan |
