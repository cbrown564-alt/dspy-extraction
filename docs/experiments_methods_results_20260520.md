...
Artifact index: `runs/`, `configs/experiments/`, `docs/kanban_frozen_threads_history.md`.
...
All model-backed work was preceded by a **deterministic foundation milestone** (loaders, gold-source policies, normalizers, split metadata, scorers, run artifacts) so that metric changes could not arise from silent label reinterpretation (`docs/deterministic_foundation_decisions.md`, `docs/deterministic_scorer_semantics.md`).
...
Local Qwen used LiteLLM `ollama_chat/` with reasoning disabled. ExECT Qwen configs raised `max_tokens` and `num_ctx` versus Gan direct defaults. Per `docs/qwen_dspy_latency_policy.md`, **ChainOfThought + BootstrapFewShot on Qwen3.6:35b** was excluded from routine evaluation after truncation and latency failures.
...
5. **Pre-registered promotion gates** for Gan temporal port (monthly within 1 pp of prior anchor, evidence ≥ prior anchor, schema ≥ 99%; `docs/gan_s0_gpt4_1_mini_temporal_candidates_full_validation_decision_20260520.md`).
...
On S0/S1, Qwen **matched or exceeded** GPT diagnosis F1 (95.1% vs 93.8%) but **underperformed on seizure-type F1** (55.7% vs 90.5%), explaining most of the micro F1 gap. S2 and S3 Qwen replication tracked GPT within **~2 pp** at the micro average. S4 Qwen cap-25/full was in progress when this document was drafted (`docs/exect_qwen35b_ladder_replication_plan.md`).
