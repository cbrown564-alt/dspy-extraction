...
Related: `docs/planning/kanban_plan.md`, `docs/experiments/exect/exect_negative_probe_synthesis_20260520.md`, `docs/taxonomy/taxonomy_primitive_coverage_audit_20260520.md`, `docs/taxonomy/experiment_taxonomy_schema.md`, `docs/taxonomy/experiment_taxonomy_research_synthesis_20260520.md`, `docs/workstreams/hybrid/hybrid_component_taxonomy_decision_20260520.md`, `docs/experiments/exect/exect_field_family_deterministic_support_map_20260520.md`
...
- Card 16 registry location decided in `docs/taxonomy/taxonomy_primitive_contract.md`: typed Python metadata is the source of truth; Markdown remains the inspection surface.
- Card 2 seed catalog created in `docs/taxonomy/taxonomy_primitive_catalog.md`.
...
- Card 11 broad ExECT family primitive backlog sketched in `src/clinical_extraction/exect/family_backlog.py` and `docs/taxonomy/taxonomy_primitive_catalog.md`.
...
- Card 17 tool-interface boundary decision recorded in `docs/taxonomy/taxonomy_tool_interface_decision_20260520.md`: minimal adapter-level H3 support only; no broad tool-wrapper framework.
- Card 18 ontology/CUI scope decision recorded in `docs/taxonomy/taxonomy_ontology_cui_scope_decision_20260520.md`: defer CUI-aware primitives to benchmark-reproduction phase; CUIPhrase bridges remain string-surface canonicalization.
...
Outcome: `docs/taxonomy/taxonomy_primitive_coverage_audit_20260520.md` classifies implemented and planned primitives as `promoted`, `diagnostic_only`, `rejected_for_current_arm`, `planned`, or `blocked`, tied to the inspection docs and comparison groups that justify the status.

Dependencies: Cards 1-18 and 22  
Parallelizable: yes  
Owner: completed 2026-05-20  
Validation: `uv run python scripts/validate_primitives.py --errors-only`; reviewed against `docs/experiments/synthesis/experiment_registry.json`, `docs/experiments/exect/exect_field_family_deterministic_support_map_20260520.md`, and the closed ExECT inspection docs.  
...
No unblocked primitive-workstream cards. Next work is the no-model Qwen S1 seizure-gap error analysis preregistered in `docs/experiments/exect/exect_qwen_s1_seizure_gap_error_analysis_preregistration_20260520.md`.
...
Notes: Implemented in `src/clinical_extraction/primitives.py`; design summarized in `docs/taxonomy/taxonomy_primitive_contract.md`.

#### Card 2 - Create Primitive Catalog Document

Outcome: `docs/taxonomy/taxonomy_primitive_catalog.md` listing current and planned primitives across Gan and ExECT, grouped by clinical operation and taxonomy position.
...
Parallelizable: no  
Owner: project  
Validation: Decision note in `docs/taxonomy/taxonomy_primitive_contract.md`.  
...
Notes: Implemented in `src/clinical_extraction/exect/primitives.py`; typed registry entries added for `exect.frequency.rate_candidates.v1` and `exect.frequency.benchmark_bridge.v1`. Tests cover quantified and qualitative candidates, Gan temporal filtering, near-miss repair, seizure-type stripping, non-audited period blocking, co-label augmentation, seizure-free prose collapse, and empty-list abstention policy. Cap-25 H2 pre-vocab remains rejected per `docs/experiments/exect/exect_s4_frequency_deterministic_gpt_inspection_20260520.md`.
...
Parallelizable: after adapters
Owner: project
Validation: Decision recorded in `docs/taxonomy/taxonomy_tool_interface_decision_20260520.md`.
...
Parallelizable: yes
Owner: project
Validation: Decision recorded in `docs/taxonomy/taxonomy_ontology_cui_scope_decision_20260520.md`.
...
Outcome: `docs/taxonomy/taxonomy_primitive_catalog.md` clearly separates implemented primitives that are reusable infrastructure from primitives that should not be used to justify rerunning rejected H2/H1 arms.
...
Dependencies: Card 23 and `docs/planning/kanban_plan.md` Phase 2 decision  
Parallelizable: no  
Owner: project  
...
Notes: Completed 2026-05-20 as a no-model Phase 2 first step rather than a model-backed run. See `docs/experiments/exect/exect_qwen_s1_seizure_gap_error_analysis_preregistration_20260520.md`. The decision selects Qwen seizure-gap error analysis using frozen artifacts before any prompt-policy, post-template repair, or synthesis-pause decision.
...
1. Execute the preregistered Qwen S1 seizure-gap error analysis and write `docs/experiments/exect/exect_qwen_s1_seizure_gap_error_analysis_20260520.md`.
...
The build-before-run phase has produced the core component library. New model-backed experiments should now be limited to cases that answer an active research decision already in `docs/planning/kanban_plan.md` and have a preregistered comparison group. The primitive workstream should favor coverage audit, catalog alignment, deterministic validation, and inspection traceability over adding more helper code.
