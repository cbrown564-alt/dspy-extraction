...
Date: 2026-05-21  
Parent plan: `docs/hybrid_pipeline_exploration_implementation_plan_20260521.md` (item 12)  
Residual queue: `data/fixtures/gan_s0_exact_frequency_residual_slice.json`

## Summary

Expanded deterministic temporal-candidate builders in `src/clinical_extraction/gan/temporal_candidates.py` to target arithmetic/window, unknown-vs-quantified, cluster-composition, and long-denominator failure modes identified in `docs/gan_s0_exact_frequency_residual_manual_read_20260521.md`.
...
| --- | ---: | ---: |
| Gold label in deterministic candidates | 1/30 (`gan_15306`) | **19/30 (63%)** |
...
| --- | --- | --- |
| Highest-frequency seizure-type selection | `gan_12562`, `gan_12667`, `gan_12679` | Competing seizure types; not addressable by window arithmetic alone |
| Unknown (unmatched phrasing) | `gan_10751`, `gan_14081`, `gan_14092` | Need additional anchor patterns |
| Long-window cluster labels | `gan_15240`, `gan_15255` | `multiple cluster per N month` phrasing not yet built |
...
Inspection: `docs/gan_s0_exact_frequency_slot_payload_residual_slice_replay_v2_20260521.md`
...
**Cap-50 confirm (2026-05-21):** `docs/gan_s0_expanded_builders_prose_gpt_cap50_v1_inspection_20260521.md` — prose + expanded builders **68.0%** monthly on 50 validation records vs pre-expansion **62.0%** (+6pp); 3 recoveries (`gan_10993`, `gan_15923`, `gan_16251`), 0 regressions.

## Next steps

1. Use prose + expanded builders as default Gan S0 det-candidate surface for new runs.
2. Remaining builder gaps: long-window `multiple cluster per N month`, unmatched unknown phrasing (`gan_14081`/`gan_14092`), concurrent seizure-type priority.
3. Full-validation prereg before claiming benchmark lift; do not mechanism-close det-candidate class.
