...
def test_collect_lm_token_usage_aggregates_prompt_completion_and_total_tokens():
    ...
    usage = _collect_lm_token_usage([primary, reflection])
...
def test_collect_lm_token_usage_returns_none_when_no_usage_is_available():
    ...
    assert _collect_lm_token_usage([lm]) is None
...
""".strip()
    ...
    assert _parse_ollama_ps_output(output, "qwen3.6:35b") == "72%/28% CPU/GPU (context=4096)"
...
def test_capture_local_model_residency_returns_unavailable_when_ollama_ps_fails(tmp_path):
    ...
    residency = _capture_local_model_residency("ollama", "qwen3.6:35b", runner=runner)
