...
def test_build_gan_s0_module_dispatches_program_variants():
    ...
    assert isinstance(
        build_gan_s0_module(GAN_FREQUENCY_S0_SPECIAL_CLASS_TARGET_SELECTOR_VARIANT),
        GanFrequencyS0SpecialClassTargetSelectorModule,
    )
    assert isinstance(
        build_gan_s0_module(
...
def test_gan_s0_special_class_target_selector_records_semantic_class_and_date_payload():
    record = next(r for r in load_gan_records() if r.record_id == "gan_13123")
    selected_evidence = "no seizures for nearly a year"
    candidate = GanTemporalFrequencyCandidate(
        canonical_label="1 per year",
    ...
    _configure_dummy([
        {
            "adjudication_json": (
                '{"target_semantic_class": "frequency_present_quantified", '
                '"reason_code": "select_current_quantified_rate", '
                '"selected_candidate_index": 1, '
                '"selected_candidate_label": "1 per year", '
                f'"selected_evidence_text": {selected_evidence!r}, '
    ...
    module = GanFrequencyS0SpecialClassTargetSelectorModule()
    with _patch_temporal_candidates([candidate]):
        prediction_set = predict_gan_records(
            module,
            [record],
            model_provider="mock",
            model_name="dummy-fixture",
            program_variant=GAN_FREQUENCY_S0_SPECIAL_CLASS_TARGET_SELECTOR_VARIANT,
        )

    pred = prediction_set.predictions[0]
    assert pred.values[0].raw_value == "1 per year"
    assert pred.metadata["target_semantic_class"] == "frequency_present_quantified"
    assert pred.metadata["target_selection_reason_code"] == (
        "select_current_quantified_rate"
    )
    assert pred.metadata["reason_code_adjudication"]["target_semantic_class"] == (
        "frequency_present_quantified"
    )
    assert pred.metadata["selected_candidate_reference"]["candidate_index"] == 1
    assert pred.metadata["temporal_candidate_source"] == (
